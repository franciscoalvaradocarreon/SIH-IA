...
import type { RolForm as RolFormType } from '../types';
import { MdSave, MdCancel, MdSecurity } from 'react-icons/md';
...
const RolForm: React.FC = () => {
  ...
  const [form, setForm] = useState<RolFormType>({
    nombre: '',
    descripcion: '',
  });
  ...
  const cargarRol = async () => {
    setLoading(true);
    try {
      const res = await rolService.obtener(Number(id));
      setForm(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        await rolService.actualizar(Number(id), form);
      } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <MdSecurity className="text-gray-400 dark:text-gray-500 text-lg" />
              </div>
              <input
                type="text"
                name="nombre"
                ...
                placeholder="Ej: ADMIN, USER, COORDINADOR"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              onChange={handleChange}
              rows={4}
              className="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-none"

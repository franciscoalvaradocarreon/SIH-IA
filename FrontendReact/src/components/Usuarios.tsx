...
const Usuarios: React.FC = () => {
  ...
  const cargarUsuarios = async () => {
    setLoading(true);
    try {
      const res = await usuarioService.listar(page, size, busqueda);
  ...
  const handleEliminar = (usuario: Usuario) => {
    setModalEliminar({ abierto: true, usuario });
  };
  ...
  const confirmarEliminar = async () => {
    ...
    setEliminando(true);
    try {
      await usuarioService.eliminar(modalEliminar.usuario.id);
      ...
      if (esDependencia) {
        setErrorPantalla({
          titulo: 'No se puede eliminar el usuario',
          mensaje: msg,
          sugerencia: 'Desvincula primero las asignaciones o dependencias de este usuario, o desactívalo en lugar de eliminarlo.',
          tipo: 'error',
          acciones: [
            {
              label: 'Aceptar',
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() => navigate('/administracion/usuarios/new')}
              ...
              <div
                ...
                <div className="p-5">
                  <div className="flex items-center gap-3 mb-3">
                    {usuario.fotoUrl ? (
                      <img
                        src={urlFoto(usuario.fotoUrl) || undefined}
                  ...
                  <div className="space-y-1 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    {usuario.escuelas.length > 0 && (
                      <p className="text-xs text-gray-400 truncate">
                        📚 {usuario.escuelas.join(', ')}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => navigate(`/administracion/usuarios/edit/${usuario.id}`)}
      ...
      {modalEliminar.abierto && modalEliminar.usuario && (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full p-6 border border-gray-400 dark:border-gray-700">
            ...
            <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3 mb-4 text-sm">
              <div className="flex items-center gap-3 mb-2">
                {modalEliminar.usuario.fotoUrl ? (
                  <img
                    src={modalEliminar.usuario.fotoUrl}
                    alt={modalEliminar.usuario.nombreCompleto}
                  ...
                  <div className="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center text-blue-600 dark:text-blue-400 font-bold">
                    {modalEliminar.usuario.nombreCompleto.charAt(0).toUpperCase()}
                  </div>
                )}
                <div>
                  <div className="font-medium text-gray-800 dark:text-white">
                    {modalEliminar.usuario.nombreCompleto}
                  </div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    @{modalEliminar.usuario.usuario}
                  </div>
                </div>
              </div>
              <div className="flex justify-between mt-2">
                ...
                <span className="font-medium text-gray-800 dark:text-white truncate max-w-[200px]">
                  {modalEliminar.usuario.email}
                </span>
              </div>
              {modalEliminar.usuario.roles.length > 0 && (
                <div className="flex justify-between mt-1">
                  <span className="text-gray-500 dark:text-gray-400">Roles:</span>
                  <span className="font-medium text-gray-800 dark:text-white">
                    {modalEliminar.usuario.roles.join(', ')}
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setModalEliminar({ abierto: false, usuario: null })}
                disabled={eliminando}
